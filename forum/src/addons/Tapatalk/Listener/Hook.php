...
class Hook
{
    ...
    public static function get_scheme_url_params($app, $templater,  &$location, &$id_value)
    {
        ...
        $param_arr = array();
        $id_value = null;
        $rawBanner = isset($options->tapatalk_banner_control) ? $options->tapatalk_banner_control : '';
        $TT_bannerControlData = false;
        if (is_string($rawBanner) && $rawBanner !== '')
        {
            $TT_bannerControlData = @unserialize($rawBanner);
        }
        if (!is_array($TT_bannerControlData))
        {
            $TT_bannerControlData = ['banner_enable' => -1, 'forum_id' => ''];
        }
