...
class UserReportedCount extends AbstractHandler
{
	use LinkUserTrait;
	...
	public function selectData(): array
	{
		...
		return $this->db->fetchAll("
			SELECT user.user_id, user.username, COUNT(`report`.content_user_id) AS count
			FROM xf_report AS `report`
			INNER JOIN xf_user AS user ON
			    (`report`.content_user_id = user.user_id)
			WHERE
				`report`.first_report_date BETWEEN ? AND ? 
				{$reportOptionsData['query']}
            GROUP BY user.user_id
			ORDER BY COUNT(report.content_user_id) {$this->order}
			LIMIT {$this->limit}
