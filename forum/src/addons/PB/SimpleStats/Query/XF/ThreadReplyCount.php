...
class ThreadReplyCount extends AbstractHandler
{
	use LinkThreadTrait;
	use OptionNodeTrait
	{
	...
	public function getColumns(): array
	{
		return [
			...
			'count' => [
				'title' => Addon::phrase('reply_count'),
				'class' => 'dataList-cell--min',
			],
		];
	}
	...
	protected function selectData(): array
	{
		...
		return $this->db->fetchAll("
			SELECT thread.thread_id, thread.title, reply_count AS count
			FROM xf_thread AS thread
			WHERE
				thread.post_date BETWEEN ? AND ?
				AND thread.discussion_state = 'visible'
				{$nodeOptionsData['query']}
			ORDER BY reply_count {$this->order}
			LIMIT {$this->limit}
