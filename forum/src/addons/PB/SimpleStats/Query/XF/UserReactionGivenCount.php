...
class UserReactionGivenCount extends AbstractHandler
{
	use LinkUserTrait;
	use OptionUserTrait
	{
	...
	protected function selectData(): array
	{
		...
		return $this->db->fetchAll("
			SELECT user.user_id,
				user.username,
				COUNT(reaction_content.reaction_user_id) AS count
			...
			INNER JOIN xf_user AS user ON
				(reaction_content.reaction_user_id = user.user_id)
			WHERE
			...
			GROUP BY reaction_content.reaction_user_id
			ORDER BY COUNT(reaction_content.reaction_user_id) {$this->order}
			LIMIT {$this->limit}
