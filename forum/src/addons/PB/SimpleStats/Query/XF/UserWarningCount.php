...
class UserWarningCount extends AbstractHandler
{
	use LinkUserTrait;
	...
	public function selectData(): array
	{
		...
		return $this->db->fetchAll("
			SELECT user.user_id, user.username, COUNT(`warning`.user_id) AS count
			...
			ORDER BY COUNT(`warning`.user_id) {$this->order}
			LIMIT {$this->limit}
