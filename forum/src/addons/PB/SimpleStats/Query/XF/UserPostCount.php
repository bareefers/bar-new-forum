...
class UserPostCount extends AbstractHandler
{
	use LinkUserTrait;
	use OptionUserTrait
	{
	...
	public function selectData(): array
	{
		...
		return $this->db->fetchAll("
			SELECT user.user_id, user.username, COUNT(post.post_id) AS count
			...
			WHERE
				...
				AND post.post_date BETWEEN ? AND ?
				{$nodeOptionsData['query']} {$userOptionsData['query']}
            GROUP BY user.user_id
			ORDER BY COUNT(post.post_id) {$this->order}
			LIMIT {$this->limit}
