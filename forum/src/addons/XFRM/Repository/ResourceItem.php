...
class ResourceItem extends Repository
{
	 ...
	 *
	 * @deprecated Use the content-agnostic feature system instead
	 */
	...
	/**
	 * @param Category|null $withinCategory If provided, applies category-specific limits
	 *
	 * @return \XFRM\Finder\ResourceItem
	 */
	public function findResourcesForApi(?Category $withinCategory = null)
	{
		/** @var \XFRM\Finder\ResourceItem $finder */
		$finder = $this->finder('XFRM:ResourceItem')
			->with('api');
	...
	public function findResourceForThread(Thread $thread)
	{
		/** @var \XFRM\Finder\ResourceItem $finder */
		$finder = $this->finder('XFRM:ResourceItem');
	...
	public function getAvailableCurrencies(?\XFRM\Entity\ResourceItem $resource = null)
	{
		...
		{
			$currency = Str::strtoupper(Str::substr($currency, 0, 3));
		...
		{
			$currency = Str::strtoupper(Str::substr($resource->currency, 0, 3));
			if (isset($currencyData[$currency]))
			{
				$output[$currency] = $currencyData[$currency];
			}
	...
	public function batchUpdateResourceViews()
	{
		...
		$viewMetrics = $db->fetchAll(
			'SELECT resource.resource_id AS content_id,
					resource.resource_date AS content_date,
					resource.resource_category_id AS content_container_id,
					resource_view.total AS view_count
				FROM xf_rm_resource_view AS resource_view
				INNER JOIN xf_rm_resource AS resource
					ON (resource.resource_id = resource_view.resource_id)'
		);
		...
		$activityLogRepo->bulkLog('resource', $viewMetrics);
