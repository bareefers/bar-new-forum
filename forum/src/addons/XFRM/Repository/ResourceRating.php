...
{
	public function findReviewsInResource(ResourceItem $resource, array $limits = [])
	{
	...
	public function findLatestReviewsForApi(?Category $withinCategory = null)
	{
		/** @var \XFRM\Finder\ResourceRating $finder */
		$finder = $this->finder('XFRM:ResourceRating');
	...
	public function getResourceRatingSplit(ResourceItem $resource): array
	{
		$splitRatings = $this->app()->db()->fetchAllKeyed("
 			...
 			GROUP BY rating
 		", 'rating', [$resource->rating_count, $resource->resource_id]);

		$stats = array_fill_keys(range(5, 1), [
			'percentage' => 0,
			'total' => 0,
		]);

		foreach ($splitRatings AS $rating => $value)
		{
			$stats[$rating]['percentage'] = $value['percentage'];
			$stats[$rating]['total'] = $value['total'];
		}
