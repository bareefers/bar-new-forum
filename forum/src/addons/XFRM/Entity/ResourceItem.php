...
use XF\Util\Arr;
use XFRM\Repository\ResourceRating;
 ...
 * @property string $external_url_
 * @property string $external_purchase_url
 * @property string $external_purchase_url_
 ...
 * @property array|null $rating_split
 * @property int $update_count
 * @property int $review_count
 * @property int $last_update
 * @property string $alt_support_url
 * @property string $alt_support_url_
 * @property array|null $custom_fields_
 * @property int $prefix_id
 * @property int $icon_date
 * @property bool $icon_optimized
 * @property array|null $tags
 * @property bool $featured
 *
 * GETTERS
 * @property Set $custom_fields
 * @property-read int $real_update_count
 * @property-read int $real_review_count
 * @property-read array $resource_update_ids
 * @property-read array $resource_version_ids
 * @property-read array $resource_rating_ids
 * @property-read Draft $draft_update
 * @property-read string|null $cover_image
 *
 * RELATIONS
 * @property-read Category|null $Category
 * @property-read User|null $User
 * @property-read AbstractCollection<ResourceTeamMember> $TeamMembers
 * @property-read ResourceUpdate|null $Description
 * @property-read ResourceVersion|null $CurrentVersion
 * @property-read Thread|null $Discussion
 * @property-read ResourcePrefix|null $Prefix
 * @property-read AbstractCollection<ResourceWatch> $Watch
 * @property-read DeletionLog|null $DeletionLog
 * @property-read ApprovalQueue|null $ApprovalQueue
 * @property-read AbstractCollection<\XF\Entity\Draft> $DraftUpdates
 * @property-read AbstractCollection<BookmarkItem> $Bookmarks
 * @property-read FeaturedContent|null $Feature
 */
class ResourceItem extends Entity implements ContainableInterface, DatableInterface, LinkableInterface, ViewableInterface
{
	use BookmarkTrait;
	...
	protected const RATING_WEIGHTED_THRESHOLD = 10;
	protected const RATING_WEIGHTED_AVERAGE = 3;
	...
	public function getStructuredData(): array
	{
		...
		$resourceLink = $this->getContentUrl(true);
	...
	{
		return Draft::createFromEntity($this, 'DraftUpdates');
	}
	...
	public function getCoverImage(): ?string
	{
		...
		$attachments = $description->attach_count
			? $description->Attachments
			: $this->_em->getEmptyCollection();

		return $this->getCoverImageInternal(
			$attachments,
			$this->canViewUpdateImages(),
			$description->embed_metadata,
			$description->message
		);
	}
	...
	public function rebuildRating()
	{
		...
		$this->rating_split = \XF::repository('XFRM:ResourceRating')->getResourceRatingSplit($this);
	}
		...
		if ($this->isUpdate())
		{
			...
			if ($this->isChanged('discussion_thread_id'))
			{
				...
				if (
					...
					&& $this->Discussion
					&& $this->Discussion->discussion_type === AbstractHandler::BASIC_THREAD_TYPE
				)
				{
	...
	protected function resourceReassigned()
	{
		...
		$this->db()->update(
			...
			[
				'author_response_team_user_id' => $this->user_id,
				'author_response_team_username' => $this->username,
			],
			'resource_id = ? AND author_response_team_user_id = ?',
			[$this->resource_id, $this->getExistingValue('user_id')]
		);
		$this->db()->update(
			'xf_rm_resource_update',
			[
				'team_user_id' => $this->user_id,
				'team_username' => $this->username,
			],
			'resource_id = ? AND team_user_id = ?',
			[$this->resource_id, $this->getExistingValue('user_id')]
		);
		$this->db()->update(
	 ...
	 * @api-out str $description <verbose>
	 * @api-out str $description_parsed <verbose> HTML parsed version of the description
	 * @api-out int $description_attach_count <verbose>
	 * @api-out Attachment[] $DescriptionAttachments <verbose|cond> Attachments to the description, if any
	 * @api-out int $reaction_score <verbose>
	 * @api-out int $update_count <verbose>
	 * @api-out int $review_count <verbose>
	 * @api-out str $current_download_url <verbose|cond> For downloadable resources with an external download URL
	 * @api-out object[] $current_files <verbose|cond> For downloadable resources with local files
	 * @api-out str $external_purchase_url <verbose|cond> For externally purchasable resources
	 * @api-out bool $is_watching <cond> If visitor is logged in
	 * @api-out str $icon_url <cond> If resource icons are enabled
	 * @api-out str $version <cond> Current version string, if the resource is versioned
	 * @api-out object $custom_fields Key-value pairs of custom field values
	 * @api-out str[] $tags
	 * @api-out str $prefix <cond> Rendered prefix title, if a prefix is set
	 ...
	 * @api-out bool $can_view_description_attachments
	...
	)
	{
		...
		if ($this->prefix_id)
		{
			$result->prefix = \XF::phrase('resource_prefix.' . $this->prefix_id)->render();
		}
	...
	public static function getStructure(Structure $structure)
	{
		...
		$structure->columns = [
			...
			'rating_split' => ['type' => self::JSON_ARRAY, 'default' => []],
			...
			'icon_optimized' => ['type' => self::BOOL, 'default' => false],
