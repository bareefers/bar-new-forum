 ...
 * @property string $last_resource_title_
 * @property int $last_resource_id
 * @property array|null $field_cache
 * @property array|null $review_field_cache
 ...
 *
 * GETTERS
 * @property-read ArrayCollection $prefixes
 * @property-read Draft $draft_resource
 *
 * RELATIONS
 * @property-read Forum|null $ThreadForum
 * @property-read AbstractCollection<CategoryWatch> $Watch
 * @property-read AbstractCollection<\XF\Entity\Draft> $DraftResources
...
class Category extends AbstractCategoryTree implements LinkableInterface, ViewableInterface
{
	...
	{
		return Draft::createFromEntity($this, 'DraftResources');
	}
	...
	public function rebuildCounters()
	{
		...
		$this->featured_count = $this->db()->fetchOne(
			"SELECT COUNT(*)
				FROM xf_featured_content AS featured_content
				INNER JOIN xf_rm_resource AS resource
					ON (resource.resource_id = featured_content.content_id)
				WHERE featured_content.content_type = 'resource'
					AND featured_content.content_container_id = ?
	...
	/**
	 * @api-out XFRM_ResourcePrefix[] $prefixes <verbose>
	 * @api-out object[] $custom_fields <verbose>
