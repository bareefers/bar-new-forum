 ...
 * @property-read float|int $rating_avg
 *
 * RELATIONS
 * @property-read ResourceItem|null $Resource
 * @property-read User|null $TeamUser
 * @property-read AbstractCollection<\XF\Entity\Attachment> $Attachments
 * @property-read AbstractCollection<ResourceDownload> $Downloads
 * @property-read AbstractCollection<ResourceRating> $Ratings
 * @property-read DeletionLog|null $DeletionLog
 * @property-read ApprovalQueue|null $ApprovalQueue
 */
class ResourceVersion extends Entity implements LinkableInterface, ViewableInterface
{
	...
	public function getExternalDownloadDomain()
	{
		...
		$displayDomain = idn_to_utf8(
			$domain,
			IDNA_DEFAULT,
			$variant
		);
		return $displayDomain !== false ? $displayDomain : $domain;
	}
	 ...
	 * @api-out str $download_url <cond> For versions with an external download URL
	 * @api-out object[] $files <cond> For versions with local download files
	 * @api-out num $rating_avg
