...
class Setup extends AbstractSetup
{
	...
	public function upgrade901000570Step2(array $stepParams)
	{
		...
		foreach ($albumIds AS $albumId)
		{
			...
			$ownerShared = $db->fetchOne(
				'SELECT shared_user_id FROM xengallery_shared_map WHERE album_id = ? AND shared_user_id = ?',
				$bind
			);
			if (!$ownerShared)
			{
	...
	public function upgrade902000010Step6()
	{
		...
		if ($watermarkId)
		{
			$existingPath = sprintf(
				'data://xengallery_watermark/%d/%d.jpg',
	...
	public function upgrade902000010Step25(array $stepParams)
	{
		...
		foreach ($albumIds AS $albumId)
		{
			...
			if ($album)
			{
				...
				if ($mediaItem)
				{
					$mediaThumbAbsPath = sprintf(
						'data://xfmg/thumbnail/%d/%d-%s.jpg',
						floor($mediaItem['media_id'] / 1000),
						$mediaItem['media_id'],
						$mediaItem['media_hash']
					);
					$albumThumbAbsPath = sprintf(
						'data://xfmg/album_thumbnail/%d/%d-%s.jpg',
	...
	{
		$timer = new Timer($this->app->config('jobMaxRunTime'));
	...
	public function upgrade902010010Step2(array $stepParams)
	{
		...
		return $this->entityColumnsToJson(
			'XFMG:Category',
			['field_cache', 'allowed_types', 'breadcrumb_data'],
			$position,
			$stepData
		);
	}
	...
	public function upgrade902030010Step1()
	{
		$db = $this->db();
		...
		$attachmentExtensions = $db->fetchOne('
			SELECT option_value
			FROM xf_option
			WHERE option_id = \'xfmgImageExtensions\'
		');
	...
	public function upgrade902030010Step2()
	{
		$this->alterTable('xf_mg_category', function (Alter $table)
		{
			$table->addColumn('auto_feature', 'tinyint')->setDefault(0)->after('category_index_limit');
		});
	}

	public function upgrade902030010Step3()
	{
		$this->alterTable('xf_mg_media_item', function (Alter $table)
		{
	...
	public function upgrade902030010Step4(): void
	{
		$tables = [
			'xf_mg_album_view',
			'xf_mg_media_view',
		...
		{
			\XF::logError('During upgrade to XenForo Media Gallery 2.3.0 we could not convert some tables to InnoDb: ' . implode(', ', $tables));
			return;
		}
	...
	public function upgrade902030036Step1()
	{
		$this->alterTable('xf_mg_media_item', function (Alter $table)
		{
			$table->addColumn('album_state', 'enum')->values(['visible','moderated','deleted'])->nullable()->after('album_id');
		});
	}

	public function upgrade902030036Step2(array $stepParams)
	{
		$stepParams = array_replace([
			'position' => 0,
		], $stepParams);
		...
		foreach ($mediaIds AS $mediaId)
		{
			...
			$db->query("
				UPDATE xf_mg_media_item AS media
				INNER JOIN xf_mg_album AS album ON
					(media.album_id = album.album_id)
				SET media.album_state = album.album_state
				WHERE media_id = ?
			", $mediaId);
	...
	public function upgrade902030055Step1(): void
	{
		$this->insertNamedWidget('xfmg_wrapper_trending_media');
	}
	...
	public function uninstallStep1()
	{
		...
		foreach (array_keys($this->getDefaultWidgetSetup()) AS $widgetKey)
		{
	...
	{
		return [
			'xfmg_wrapper_trending_media' => function ($key, array $options = [])
			{
	...
	protected function applyDefaultPermissions($previousVersion = null)
	{
		...
		if (!$previousVersion || $previousVersion < 902030010)
		{
			// XFMG: Media moderator permissions
			$this->applyGlobalPermission('xfmg', 'featureUnfeature', 'forum', 'stickUnstickThread');
