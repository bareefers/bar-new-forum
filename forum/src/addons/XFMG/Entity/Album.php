...
use XF\Util\Arr;
use XF\Util\File;
use XFMG\Service\Album\ThumbnailGenerator;
 ...
 * @property array|null $media_item_cache
 * @property string|null $view_privacy
 * @property array|null $view_users
 * @property string|null $add_privacy
 * @property array|null $add_users
 ...
 *
 * GETTERS
 * @property-read array $allowed_types
 * @property-read array $field_cache
 * @property-read int $min_tags
 * @property-read string|null] $thumbnail_url
 * @property-read ArrayCollection $MediaCache
 ...
 * @property-read AbstractCollection<SharedMapAdd> $SharedMapAdd
 * @property-read AbstractCollection<SharedMapView> $SharedMapView
 * @property-read ApprovalQueue|null $ApprovalQueue
 * @property-read AbstractCollection<AlbumWatch> $Watch
 ...
 * @property-read AbstractCollection<AlbumCommentRead> $CommentRead
 ...
 */
class Album extends Entity implements LinkableInterface, ViewableInterface
{
	use BookmarkTrait;
	...
	public function getThumbnailUrl($canonical = false)
	{
		...
		$path = sprintf(
			"xfmg/album_thumbnail/%d/%d-%s.jpg?{$this->thumbnail_date}",
			floor($albumId / 1000),
			$albumId,
			$this->album_hash
		);
	...
	public function getStructuredData()
	{
		...
		$albumLink = $this->getContentUrl(true);
		...
		$mainEntity = [
			...
			'dateModified' => $this->last_update_date
				? gmdate('c', $this->last_update_date)
				: null,
			'url' => $albumLink,
			...
			'interactionStatistic' => [
				[
					...
					'userInteractionCount' => $this->media_count,
				],
				[
					'@type' => 'InteractionCounter',
	...
	protected function _postSave()
	{
		...
		{
			$this->db()->update('xf_mg_media_item', ['album_state' => $this->album_state], 'album_id = ?', $this->album_id);
		}
