...
class Media extends Repository
{
	...
	public function findMediaForUser(User $user, $categoryIds = null, array $limits = [])
	{
		$limits['categoryIds'] = $categoryIds;
	...
	public function findMediaForApi($typeLimit = null, ?Category $category = null)
	{
		/** @var \XFMG\Finder\MediaItem $finder */
		$finder = $this->finder('XFMG:MediaItem');
	...
	public function getCurrentPositionInCategory(MediaItem $mediaItem, Category $category, array $limits = [])
	{
	...
	public function getCurrentPositionInAlbum(MediaItem $mediaItem, Album $album, array $limits = [])
	{
	...
	public function getMediaTypeFromAttachment(Attachment $attachment)
	{
	...
	public function batchUpdateMediaViews()
	{
		...
		$viewMetrics = $db->fetchAll(
			'SELECT media.media_id AS content_id,
					media.media_date AS content_date,
					media.category_id AS content_container_id,
					media_view.total AS view_count
				FROM xf_mg_media_view AS media_view
				INNER JOIN xf_mg_media_item AS media
					ON (media.media_id = media_view.media_id)'
		);
		...
		$activityLogRepo->bulkLog('xfmg_media', $viewMetrics);
	...
	public function markMediaViewedByVisitor($categoryIds = null, $newViewed = null)
	{
		...
		$mediaItems = $finder->fetchDeferred();
	...
	public function getWatermarkAsTempFile()
	{
		...
		return File::copyAbstractedPathToTempFile(
	...
	public function rebuildImageThumb(MediaItem $mediaItem, $log = true)
	{
		...
		if ($imageManager->canResize($attachData->width, $attachData->height))
		{
			...
			/** @var ThumbnailGenerator $generatorService */
	...
	public function rebuildFFmpegPoster(MediaItem $mediaItem, $log = true)
	{
		...
		try
		{
			if ($mediaItem->custom_thumbnail_date)
			{
				...
				$tempPoster = File::copyAbstractedPathToTempFile($thumbOrigPath);
			}
			else
			{
	...
	public function rebuildCustomThumbnail(MediaItem $mediaItem, $log = true)
	{
		...
		$tempFile = File::copyAbstractedPathToTempFile($origCustomThumbPath);
		...
		if ($imageManager->canResize($width, $height))
		{
			...
			try
			{
				File::copyFileToAbstractedPath($tempThumb, $customThumbPath);
				return true;
			}
			catch (\Exception $e)
			{
