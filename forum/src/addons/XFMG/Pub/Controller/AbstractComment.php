...
abstract class AbstractComment extends AbstractController
{
	 ...
	 *
	 * @return MediaItem | \XFMG\Entity\Album
	 */
	 ...
	 * @return Creator
	 */
	protected function setupCommentCreate(Entity $content)
	{
	...
	protected function finalizeCommentCreate(Creator $creator)
	{
		...
		if (!$isPreRegComment)
		{
			if ($this->filter('no_captcha', 'bool')) // JS is disabled so user hasn't seen Captcha.
			{
				...
				return $this->rerouteController(
					$content->content_type == 'xfmg_media' ? 'XFMG:MediaComment' : 'XFMG:AlbumComment',
					'comment',
					$params
				);
			}
	...
	protected function getPreRegCommentActionData(Creator $creator)
	{
		$comment = $creator->getComment();
