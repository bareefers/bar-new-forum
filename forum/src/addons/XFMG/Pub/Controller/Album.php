...
use XFMG\Service\Album\Approver;
...
class Album extends AbstractController
{
	...
	public function actionView(ParameterBag $params)
	{
		...
		if ($this->responseType == 'rss')
		{
			...
			$feedLink = $this->buildLink('canonical:media/albums/index.rss', $album);
	...
	protected function finalizeAlbumEdit(Editor $editor)
	{
	}
	...
	public function actionBookmark(ParameterBag $params)
	{
		...
		return $bookmarkPlugin->actionBookmark(
			$album,
			$this->buildLink('media/albums/bookmark', $album)
		);
	}
	...
	public static function resolveToEmbeddableContent(ParameterBag $params, RouteMatch $routeMatch): ?Entity
	{
		...
		if ($params->album_id)
		{
			...
			$content = \XF::em()->find('XFMG:Album', $params->album_id);
		}
