...
class Forum extends AbstractNode
{
	...
	/**
	 * Determines if the given thread meets the search engine indexability criteria for this forum.
	 * This is primarily to make it easier to check a thread's default state, ignoring any override.
	 * Note that the thread being checked doesn't have to be in this forum and could be used as a
	 * hypothetical.
	 *
	 * @param Thread $thread
	 *
	 * @return bool
	 */
	public function doesThreadMeetSearchEngineIndexCriteria(Thread $thread): bool
	{
		if ($this->allow_index == 'criteria')
		{
			$criteria = $this->index_criteria;

			if (!empty($criteria['is_sticky']) && $thread->sticky)
			{
				return true;
			}
	...
	{
		return Draft::createFromEntity($this, 'DraftThreads');
	}
	...
	public function getPrefixes()
	{
		...
		$prefixes = $this->finder(ThreadPrefixFinder::class)
	...
	public function isPrefixUsable($prefix, ?User $user = null)
	{
		...
		{
			$prefix = $this->em()->find(ThreadPrefix::class, $prefix);
			if (!$prefix)
			{
				return false;
			}
	...
	{
		$thread = $this->_em->create(Thread::class);
	...
	protected function _postDelete()
	{
		...
		{
			$this->app()->jobManager()->enqueueUnique('forumDelete' . $this->node_id, ForumDelete::class, [
		...
		{
			$this->repository(OptionRepository::class)->updateOption('reportIntoForumId', 0);
		}
	}
	...
	public function getNodeTypeApiData($verbosity = self::VERBOSITY_NORMAL, array $options = [])
	{
		...
		if ($verbosity > self::VERBOSITY_NORMAL)
		{
			...
			if ($this->field_cache)
			{
				$fieldEntities = $this->repository(ThreadFieldRepository::class)->findFieldsForList()
	...
	public static function getStructure(Structure $structure)
	{
		...
		$structure->columns = [
			...
			'auto_feature'                => ['type' => self::BOOL, 'default' => false],
