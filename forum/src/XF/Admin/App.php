...
class App extends \XF\App
{
	...
	{
		parent::setupCookieConsent($cookieConsent);

		$cookies = [
			'edit_language_id' => CookieConsent::GROUP_REQUIRED,
			'edit_style_id' => CookieConsent::GROUP_REQUIRED,
			'session_admin' => CookieConsent::GROUP_REQUIRED,
		];
	...
	protected function getPreloadExtraKeys(): array
	{
		...
		$this->fire('app_admin_registry_preload', [$this, &$keys]);
	...
	protected function renderPageHtml($content, array $params, AbstractReply $reply, AbstractRenderer $renderer)
	{
		...
		$params['actionMethod'] = 'action' . Php::camelCase($reply->getAction(), '-');
