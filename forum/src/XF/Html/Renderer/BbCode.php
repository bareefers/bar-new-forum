...
class BbCode
{
	...
	public const BR_SUBSTITUTE = "\x1A";
	 ...
	 */
	protected $_handlers = [
		'span'       => ['filterCallback' => ['$this', 'handleTagSpan']],
		...
		'tr'         => ['wrap' => "[TR]\n%s\n[/TR]", 'skipCss' => true],
		'td'         => ['filterCallback' => ['$this', 'handleTagCell']],
		'th'         => ['filterCallback' => ['$this', 'handleTagCell']],
	];
	...
	public static function renderFromHtml($html, array $options = [])
	{
		...
		$class = \XF::app()->extendClass(self::class);
		/** @var $renderer BbCode */
	...
	public function convertUrlToAbsolute($url)
	{
		...
		preg_match(
			'#^(?P<protocolHost>(?P<protocol>https?|ftp)://[^/]+)(?P<path>.*)$#i',
	...
	public function handleTagSpan($text, Tag $tag)
	{
		if ($tag->hasClass('smilie') && $tag->attribute('data-smilie'))
		{
			...
			$smilieId = intval($tag->attribute('data-smilie'));
			$smilie = $smilies[$smilieId] ?? null;

			if ($smilie && !empty($smilie['emoji_shortname']))
			{
				// if we have a short name and its valid for this smilie, return that smilie text...
				$shortname = $tag->attribute('data-shortname');
				if ($shortname && in_array($shortname, $smilie['smilieText']))
				{
					return $shortname;
				}
				// ...else, we don't know if this text is actually valid for the smilie so keep the text as is
			}
			// ...otherwise the smilie shouldn't be in this format
		}
	...
	protected function getAttachSizeAttributes(Tag $tag)
	{
		...
		if ($size = $tag->attribute('data-size'))
		{
			$attr .= " size=\"{$size}\"";
		}
	...
	{
		$width = $tag->attributes()['style']['width'] ?? null;
	...
	public function handleTagCell($text, Tag $tag)
	{
		$tagName = $tag->tagName();
		$width = $tag->attributes()['style']['width'] ?? null;
		...
		if ($width)
		{
			$output = "[$tagName width=\"$width\"]{$text}[/$tagName]";
		}
		else
		{
	...
	{
		// workaround for [COLOR=hsl(var(--xf-editorFocusColor))] Froala bug
		if (strpos($color, 'var(') !== false || strpos($color, '--xf-') !== false)
		{
			return $text;
		}
