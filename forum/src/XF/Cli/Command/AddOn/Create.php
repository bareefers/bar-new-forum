...
class Create extends AbstractCommand
{
	...
	protected function execute(InputInterface $input, OutputInterface $output)
	{
		...
		$versionString = \XF::repository(AddOnRepository::class)->inferVersionStringFromId($versionId);
		if ($versionString)
		{
		...
		if ($legacyAddOnId)
		{
			$addOn = \XF::em()->find(\XF\Entity\AddOn::class, $legacyAddOnId);
			if ($addOn)
			{
				$renamedLegacy = true;
			}
			else
			{
				...
				$addOn = \XF::em()->create(\XF\Entity\AddOn::class);
			}
		}
		else
		{
		...
		if ($written)
		{
			$addOn->fastUpdate('json_hash', Hash::hashTextFile($jsonPath, 'sha256'));
