...
class Html extends AbstractRenderer
{
	...
	// anchor => number of occurrences
	protected $anchorOccurrences = [];
	...
	public function renderTag(array $tag, array $options)
	{
		...
		else if (!empty($renderRule['replace']))
		{
			...
			{
				[$prepend, $append] = $renderRule['replace'];
	...
	public function filterString($string, array $options)
	{
		...
		if ($allowSmilies)
		{
			$string = $this->formatter->replaceSmiliesHtml($string, $options['smilieFormat']);
			if ($options['replaceEmojis'])
			{
				$string = $this->formatter->getEmojiFormatter()->formatEmojiToImage(
					$string,
					['wrapNative' => true]
				);
			}
	...
	public function renderTagCode(array $children, $option, array $tag, array $options)
	{
		...
		/** @var CodeLanguage $codeLanguages */
		$codeLanguages = \XF::app()->data(CodeLanguage::class);
	...
	public function renderTagImage(array $children, $option, array $tag, array $options)
	{
		...
		if (empty($displayAttrs['width']) && empty($displayAttrs['height']))
		{
			$hash = md5($validUrl);
			$image = $options['images'][$hash] ?? null;
	...
	public function renderTagHeading(array $children, $option, array $tag, array $options)
	{
		...
		$anchor = trim($this->renderSubTreePlain($children));
		$anchor = $this->formatter->stripBbCode($anchor);
		$router = \XF::app()->router('public');
		$anchor = $router->prepareStringForUrl($anchor);
		$anchor = '-' . $anchor;

		if ($anchor && !empty($options['showHeadingAnchors']))
		{
			if (isset($this->anchorOccurrences[$anchor]))
			{
				$this->anchorOccurrences[$anchor]++;
				$anchor .= '-' . $this->anchorOccurrences[$anchor];
			}
			else
			{
				$this->anchorOccurrences[$anchor] = 1;
			}

			$title = htmlspecialchars(\XF::phrase('permanent_link'));

			return $this->wrapHtml(
				"<$tag class=\"bbHeading\"><a class=\"u-anchorTarget\" name=\"$anchor\"></a>",
				$output . $invisibleSpace,
				"<a class=\"hoverLink\" href=\"#$anchor\" title=\"$title\"></a></$tag>"
			);
		}
		else
		{
			return $this->wrapHtml("<$tag class=\"bbHeading\">", $output . $invisibleSpace, "</$tag>");
		}
	}
	...
	protected function prepareTextFromUrlExtended($url, array $options)
	{
		...
		{
			$length = Str::strlen($text);
			if ($length > 100)
			{
				$text = Str::substr_replace($text, '...', 35, $length - 35 - 45);
			}
		}
	...
	protected function renderTableCell(array $tag, array $options)
	{
		...
		if ($width)
		{
			return "<$tag[tag] style='width: " . htmlspecialchars($width, ENT_QUOTES) . "'>$output</$tag[tag]>";
		}
		else
		{
	...
	public function renderTagEmbed(array $children, $option, array $tag, array $options): string
	{
		if (!empty($options['withinEmbedResolver']))
		{
			return '';
		}

		[$contentType, $contentId] = \XF::app()->getContentTypeIdFromString($option['content'] ?? '');

		$embeds = $options['embeds'] ?? [];

		$url = is_string($children[0] ?? null) ? $children[0] : '';
		$censored = $this->formatter->censorText($url);
		$validUrl = $this->getValidUrl($url);

		if ($url && $validUrl && $censored === $url)
		{
			$text = $this->prepareTextFromUrlExtended($validUrl, $options);
			$fallback = '<div>' . $this->getRenderedLink($text, $validUrl, $options) . '</div>';
		}
		else
		{
			$fallback = $this->renderUnparsedTag($tag, $options);
		}

		if (!isset($embeds[$contentType][$contentId]))
		{
			return $fallback;
		}

		/** @var Entity|EmbedResolverTrait $content */
		$content = $embeds[$contentType][$contentId];
		...
		return $this->templater->renderTemplate('public:bb_code_tag_embed', [
			'content' => $content,
			'contentUrl' => $url,
			'fallback' => $fallback,
		]);
