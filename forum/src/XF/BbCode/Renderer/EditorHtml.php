...
class EditorHtml extends Html
{
	...
	public function filterFinalOutput($output)
	{
		...
		if ($debug)
		{
			echo '<hr /><b>Original:</b><br />' . nl2br(htmlspecialchars($output));
		}
		...
		if ($debug)
		{
			echo '<hr /><b>Post-break:</b><br />' . nl2br(htmlspecialchars($output));
		}
		...
		if ($debug)
		{
			echo '<hr /><b>Final:</b><br />' . nl2br(htmlspecialchars($output));
		}
	...
	protected function getRenderedImg($imageUrl, $validUrl, array $params = [])
	{
		...
		return '<img src="' . htmlspecialchars($validUrl) . '"'
			...
			. ' data-size="' . htmlspecialchars($params['size'] ?? '') . '"'
