...
use function array_key_exists, array_slice, call_user_func_array, count, get_class, is_array, is_int, is_string, strlen;

abstract class Entity implements \ArrayAccess
{
	public const REQUIRES_DECODING = 0x10000;
	...
	public const LIST_LINES            = 0x10011;
	public const LIST_COMMA            = 0x10012;
	...
	public const LIST_ARRAY            = 0x10013;

	/** @deprecated - use JSON_ARRAY instead  */
	public const SERIALIZED_ARRAY      = 0x10008;

	public const TO_ONE  = 1;
	public const TO_MANY = 2;
	...
	public const MATCH_URL_EMPTY = 'url_empty';
	public const MATCH_EMAIL = 'email';
	public const MATCH_EMAIL_EMPTY = 'email_empty';

	public const MATCH_ALPHANUMERIC        = '^[A-Za-z0-9_]*$';
	public const MATCH_ALPHANUMERIC_HYPHEN = '^[A-Za-z0-9_-]*$';
	public const MATCH_ALPHANUMERIC_DOT    = '^[A-Za-z0-9_.]*$';
	...
	final public function toApiResult($verbosity = self::VERBOSITY_NORMAL, array $options = [])
	{
		$result = new EntityResult($this);
		$subResult = $this->setupApiResultData($result, $verbosity, $options);

		if ($subResult instanceof EntityResultInterface)
		{
			return $subResult;
		}
		else
	...
	final public function toWebhookResult($verbosity = self::VERBOSITY_NORMAL, array $options = [])
	{
		$result = new EntityResult($this, EntityResultInterface::TYPE_WEBHOOK);
		$subResult = $this->setupApiResultData($result, $verbosity, $options);
	...
	protected function _verifyValueCustom(&$value, $key, $type, array $columnOptions)
	{
		...
		{
			$verifyMethod = 'verify' . Php::camelCase($key);
	...
	public function updateVersionId($versionIdField = 'version_id', $versionStringField = 'version_string', $addOnIdField = 'addon_id')
	{
		...
		if ($addOnId)
		{
			$addOn = $this->_em->find(AddOn::class, $addOnId);
			if (!$addOn)
			{
	...
	/**
	 * @template T of Behavior
	 *
	 * @param class-string<T> $behavior
	 *
	 * @return T
	 */
	...
	final public function save($throw = true, $newTransaction = true)
	{
		if ($this->_readOnly)
		{
			throw new \LogicException("Entity is read only");
		...
		if (!$this->preSave())
		{
			if ($throw)
			{
				throw new PrintableException($this->_errors);
			}
			return false;
		}
	...
	final public function saveIfChanged(&$saved = null, $throw = true, $newTransaction = true)
	{
	...
	final public function preDelete()
	{
		if ($this->_deleted)
		{
			return true;
	 ...
	 * @return array|null
	 */
	public function structureColumn($column)
	{
		if ($this->isValidColumn($column))
		{
			return $this->_structure->columns[$column];
		}
