...
class Ip
{
	...
	)
	{
		...
		if ($output === false && $throw)
		{
			throw new \InvalidArgumentException('Invalid binary IP: ' . $ip);
		}
	 ...
	 *
	 * @deprecated Use \XF\Util\Ip::stringToBinary instead
	 */
	public static function convertIpStringToBinary($ip)
	{
		...
		if (is_numeric($originalIp) && $originalIp < 2 ** 32)
		{
			// IPv4 as integer
			return static::convertHexToBin(
	 ...
	 * @return bool|string
	 *
	 * @deprecated Use \XF\Util\Ip::binaryToString instead
	 */
	...
	public static function checkIpsAgainstBinaryRangeList($checkIps, array $firstByteRangeList)
	{
		...
		{
			$binary = static::stringToBinary($ip, false);
			if (!$binary)
			{
				continue;
			}
	...
	public static function parseIpRangeString($ip)
	{
		...
		else
		{
			...
			$binary = static::stringToBinary(implode('.', $ipParts), false);
			if (!$binary)
			{
				return false;
			}
	...
	public static function getHost($ip)
	{
		...
		$decompressed = static::binaryToString($ip, false, false);
		...
		if (strpos($ip, ':') !== false)
		{
			...
			$checkIp = static::binaryToString($binary, false, false);
			if (!$checkIp)
			{
				return false;
			}
