...
templateError:".js-attachmentError",templateThumb:".js-attachmentThumb",templateView:".js-attachmentView",allowDrop:!1,resizeImages:!0,resizeImagesConvertType:!0,maxImageWidth:null,maxImageHeight:null,checkVideoSize:!0},container:null,filesContainer:null,template:null,form:null,legacyMode:!1,maxImageWidth:null,maxImageHeight:null,supportsVideoAudioUploads:null,manageUrl:null,flow:null,fileMap:{},exifMap:{},isUploading:!1,lastScroll:0,editor:null,init(){var a=this.options;const b=this.target;if(r.Flow){var c=
b.querySelectorAll(a.uploadButton);if(this.options.manageUrl)this.manageUrl=this.options.manageUrl;else{if(!c.length){console.error("No manage URL specified and no uploaders available.");return}const d=c[0];this.manageUrl=d.dataset.uploadHref||d.getAttribute("href")}this.options.maxImageWidth||this.options.maxImageHeight?(this.maxImageWidth=this.options.maxImageWidth,this.maxImageHeight=this.options.maxImageHeight):(this.maxImageWidth=XF.config.uploadMaxWidth,this.maxImageHeight=XF.config.uploadMaxHeight);
this.container=b.querySelector(a.container);this.filesContainer=b.querySelector(a.filesContainer);this.container?(XF.onDelegated(this.container,"click",a.actionButton,this.actionButtonClick.bind(this)),XF.onDelegated(this.container,"click",'input[type="checkbox"]',this.checkboxClick.bind(this)),XF.onDelegated(this.container,"click",a.selectToggleButton,this.selectToggleClick.bind(this)),XF.onDelegated(this.container,"click",a.selectActionButton,this.selectActionClick.bind(this))):(this.legacyMode=
!0,XF.onDelegated(this.filesContainer,"click",a.actionButton,this.actionButtonClick.bind(this)));(this.template=b.querySelector(a.uploadTemplate).innerHTML)||console.error("No attached file template found.");if(a=this.setupFlow()){if(this.flow=a,this.setupUploadButtons(c,a),this.options.allowDrop&&a.assignDrop([b]),setTimeout(()=>{(this.editor=XF.getEditorInContainer(this.target,"[data-attachment-target=false]"))||this.removeInsertButtons(this.container);this.toggleInsertMultiRow()},50),this.form=
this.target.closest("form"))XF.on(this.form,"ajax-submit:before",d=>{this.isUploading&&!confirm(XF.phrase("files_being_uploaded_are_you_sure"))&&(d.preventSubmit=!0)}),XF.on(this.form,"attachment-manager:reset",this.resetAttachments.bind(this))}else console.error("No flow uploader support")}else console.error("flow.js must be loaded")},setupFlow(){var a=this.getFlowOptions();a=new Flow(a);if(!a.support)return null;a.on("fileAdded",this.fileAdded.bind(this));a.on("filesSubmitted",this.filesSubmitted.bind(this));
a.on("fileProgress",this.uploadProgress.bind(this));a.on("fileSuccess",this.uploadSuccess.bind(this));a.on("fileError",this.uploadError.bind(this));return a},getFlowOptions(){return{target:this.manageUrl,allowDuplicateUploads:!0,fileParameterName:"upload",query:this.uploadQueryParams.bind(this),simultaneousUploads:1,testChunks:!1,progressCallbacksInterval:100,chunkSize:4294967296,readFileFn(a,b,c,d,e){let f="slice";a.file.slice?f="slice":a.file.mozSlice?f="mozSlice":a.file.webkitSlice&&(f="webkitSlice");
d||(d="");e.readFinished(a.file[f](b,c,d))}}},setupUploadButtons(a,b){a.forEach(c=>{var d=c.dataset.accept||"",e=XF.createElementFromString('<span class="js-attachButton"></span>');c.parentNode.insertBefore(e,c.nextSibling);e.appendChild(c);"."==d&&(d="");XF.on(c,"click",f=>f.preventDefault());b.assignBrowse(e,!1,!1,{accept:d});if(null===this.supportsVideoAudioUploads){c=XF.config.allowedVideoExtensions;const f=XF.config.allowedAudioExtensions;d=d.split(",");for(const g in d){const h=d[g].substr(1);
if(c.includes(h)||f.includes(h)){this.supportsVideoAudioUploads=!0;break}}}e=e.querySelector("input[type=file]");e.setAttribute("title",XF.htmlspecialchars(XF.phrase("attach")));e.style.overflow="hidden";e.style[XF.isRtl()?"right":"left"]="-1000px"})},fileAdded(a){var b=this.applyUploadTemplate({filename:a.name,uploading:!0});this.resizeProgress(b,0);XF.DataStore.set(b,"file",a);this.legacyMode?this.filesContainer.classList.add("is-active"):this.container.classList.add("is-active");this.filesContainer.appendChild(b);
this.fileMap[a.uniqueIdentifier]=b;var c=this.filesContainer.closest('[data-xf-init="h-scroller"]');if(c&&(c=XF.Element.getHandler(c,"h-scroller"))){const d=Date.now();this.lastScroll<d-500&&(this.lastScroll=d,c.scrollTo(XF.position(b).left-50))}this.target.querySelector(this.options.uploadButton).blur();b=this.target.querySelector(this.options.uploadButton).dataset.videoSize;if(this.options.checkVideoSize&&this.supportsVideoAudioUploads&&this.isVideoOrAudio(a)&&0<b&&a.size>b)return this.uploadError(a,
this.addErrorToJson({},XF.phrase("file_too_large_to_upload"))),!1},isVideoOrAudio(a){var b=a.name.split(".");a=XF.config.allowedVideoExtensions;const c=XF.config.allowedAudioExtensions;if(1===b.length||""===b[0]&&b.length)return!1;b=b.pop();return a.includes(b)||c.includes(b)},async filesSubmitted(a){await this.preProcessFiles(a);this.setUploading(!0);this.flow.upload()},async preProcessFiles(a){const b=async d=>{try{if(!1===await this.preProcessFile(d))throw Error("File failed pre-processing");}catch(e){this.flow.removeFile(d)}},
c=[];for(const d of a)c.push(b(d));await Promise.allSettled(c)},async preProcessFile(a){if("undefined"!==typeof ExifReader&&a.file.type.startsWith("image/")){try{var b=await ExifReader.load(a.file)}catch(c){b=null}if(b){const c={};for(const [d,e]of Object.entries(b))if("MakerNote"!==d&&(b=e.id))try{let f=Array.isArray(e.value)?e.value.join("/"):e.value;switch(d){case "ComponentsConfiguration":case "ExifVersion":case "FileSource":case "FlashpixVersion":case "InteroperabilityVersion":case "SceneType":case "UserComment":f=
e.description;break;case "GPSLatitude":case "GPSLongitude":case "GPSTimeStamp":f=Array.isArray(e.value)?e.value.map(g=>Array.isArray(g)?g.join("/"):g):e.value}c[d]={id:b,value:f}}catch(f){}this.exifMap[a.uniqueIdentifier]=c}}if(this.options.resizeImages&&a.file.type.startsWith("image/"))try{const c=await XF.ImageTools.resize(a.file,this.maxImageWidth,this.maxImageHeight,this.options.resizeImagesConvertType&&"optimize"===XF.config.imageOptimization?"image/webp":null);a.file=c;a.name=c.fileName||c.name;
a.size=c.size;a.relativePath=c.relativePath||c.webkitRelativePath||c.name;a.bootstrap();delete this.exifMap[a.uniqueIdentifier].Orientation}catch(c){}if(0<XF.config.uploadMaxFilesize&&a.size>XF.config.uploadMaxFilesize)return this.uploadError(a,this.addErrorToJson({},XF.phrase("uploaded_file_is_too_large_for_server_to_process"))),!1},uploadProgress(a){const b=this.fileMap[a.uniqueIdentifier];b&&(this.setUploading(!0),this.resizeProgress(b,a.progress()))},resizeProgress(a,b){b=Math.floor(100*b);a=
...
a.type||"image/svg+xml"===a.type)e(a);else{d=d||a.type;var g=n.createElement("img");g.onload=()=>{let h=g.width,k=g.height,p=!0;(!b||h<=b)&&(!c||k<=c)&&(p=!1);b&&h>b&&(k*=b/h,h=b);c&&k>c&&(h*=c/k,k=c);const l=n.createElement("canvas"),q=l.getContext("2d");null===q?f(Error("Failed to retrieve a valid drawing context.")):(l.width=h,l.height=k,q.drawImage(g,0,0,l.width,l.height),l.toBlob(m=>{null===m?f(Error("Failed to create blob from the canvas.")):(m=new File([m],a.name,{type:d,lastModified:a.lastModified}),
!p&&m.size>=a.size?e(a):e(m))},d,this.getQuality(a)),URL.revokeObjectURL(g.src))};g.src=URL.createObjectURL(a)}else f(Error("The file is not an image."))})}};XF.Element.register("attachment-manager","XF.AttachmentManager");XF.Element.register("attachment-on-insert","XF.AttachmentOnInsert")})(window,document);
