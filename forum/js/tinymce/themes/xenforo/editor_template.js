		...
		addButtons : function (theme, tb)
		{
			...
			button = controlManager.createColorSplitButton('xenforo_smilies', {
				...
				onselect: function(s, txt) {
					...
					txt = DOM.encode(txt) + ' '; // note this space works around a bug (#23768)

					if (s.match(/^\d+$/))
					{
						smilieHtml = '<img src="styles/default/xenforo/clear.png" alt="' + txt + '" class="mceSmilieSprite mceSmilie' + s + '" data-smilie="yes" /> ';
					}
					else
					{
						smilieHtml = '<img src="' + DOM.encode(s) + '" alt="' + txt + '" class="mceSmilie" data-smilie="yes" /> ';
					}

					editor.execCommand('mceInsertContent', false, smilieHtml);
				},
				smilies: smilies
	...
	tinymce.create('tinymce.plugins.XenForoElastic', {
		init: function(ed)
		{
			...
			eventInit = function()
			{
				...
				var $table = $iframe.closest('table');

				minHeight = Math.max(
					$table.height() - $table.find('.mceFirst').height() - 2,
					$('#' + ed.id).height() - 9
				);
...
(function(tinymce) {
	...
	tinymce.create('tinymce.ui.XenForoSmiliesButton:tinymce.ui.SplitButton', {
		...
		renderMenu : function() {
			...
			{
				var smilieHtml, dataSmilieType;

				if (typeof smilie[1] == 'number') // defines the smilie_id, so it's a sprite
				{
					smilieHtml = '<img src="styles/default/xenforo/clear.png" alt="" title="' + DOM.encode(smilie[0] + '    ' + smilieName) + '" class="mceSmilieSprite mceSmilie' + smilie[1] + '" />';
				}
				else
				{
					smilieHtml = '<img src="' + DOM.encode(smilie[1]) + '" alt="" title="' + DOM.encode(smilie[0] + '    ' + smilieName) + '" />';
				}

				DOM.add(n, 'a', {
					href : 'javascript:;',
					'data-smilieInfo' : smilie[1],
			...
			Event.add(t.id + '_menu', 'click', function(e) {
				...
				e = e.target;
				$t = $(e).closest('[data-smilieInfo]');

				if ($t.length)
				{
					t.settings.onselect($t.attr('data-smilieInfo'), $t.attr('data-smilieText'));
					t.hideMenu();
				}
...
(function(tinymce) {
	...
	tinymce.create('tinymce.themes.XenForoTheme', {
		...
		init : function(ed, url) {
			...
			var isRtl = XenForo.isRTL(),
				alignButtons = (isRtl ? 'justifyright,justifycenter,justifyleft' : 'justifyleft,justifycenter,justifyright'),
				undoRedoButtons = (isRtl ? 'redo,undo' : 'undo,redo'),
				indentOutdentButtons = (isRtl ? 'indent,outdent' : 'outdent,indent');
			...
			t.settings = s = extend({
				...
				theme_xenforo_buttons2 : 'bold,italic,underline,strikethrough,|,' + alignButtons + ',|,bullist,numlist,' + indentOutdentButtons + ',|,link,unlink,image,xenforo_media,|,xenforo_code,xenforo_custom_bbcode',
			...
			ed.onInit.add(function() {
				...
				ed.editorCommands.addCommands({Outdent : $.context(t, '_queryOutdent')}, 'state');

				var sel = 'p,h1,h2,h3,h4,h5,h6,td,th,div,ul,ol,li';
				ed.formatter.register({
					alignleft : [
						{selector : sel, styles : {textAlign : 'left'}},
						{selector : 'img,table', collapsed : false, styles : {display : 'block', marginLeft : '0', marginRight : 'auto'}}
					],
					alignright : [
						{selector : sel, styles : {textAlign : 'right'}},
						{selector : 'img,table', collapsed : false, styles : {display : 'block', marginLeft : 'auto', marginRight : '0'}}
					]
				});
			});

			ed.addShortcut('shift+ctrl+z', ed.getLang('redo_desc'), 'Redo');
...
*/
		...
		_Indent : function(ui, val) {
			return this._handleIndentOutdent('indent');
		},

		_Outdent : function(command) {
			return this._handleIndentOutdent('outdent');
		},

		_handleIndentOutdent: function(command) {
			var intentValue, indentUnit, value,
			paddingSide = (XenForo.isRTL() ? 'paddingRight' : 'paddingLeft'),
			ed = this.editor,
			selection = ed.selection,
			dom = ed.dom;

			// Setup indent level
			intentValue = ed.settings.indentation;
			...
			if (!ed.queryCommandState('InsertUnorderedList') && !ed.queryCommandState('InsertOrderedList')) {
				each(selection.getSelectedBlocks(), function(element) {
					if (command == 'outdent') {
						value = Math.max(0, parseInt(element.style[paddingSide] || 0) - intentValue);
						dom.setStyle(element, paddingSide, value ? value + indentUnit : '');
					} else
						dom.setStyle(element, paddingSide, (parseInt(element.style[paddingSide] || 0) + intentValue) + indentUnit);
				});
			} else {
				ed.getDoc().execCommand(command, false, null);
			}
		},

		_queryOutdent: function()
		{
			var node,
				paddingSide = (XenForo.isRTL() ? 'paddingRight' : 'paddingLeft'),
				ed = this.editor,
			...
			if (settings.inline_styles) {
				if ((node = dom.getParent(selection.getStart(), dom.isBlock)) && parseInt(node.style[paddingSide]) > 0)
					return true;

				if ((node = dom.getParent(selection.getEnd(), dom.isBlock)) && parseInt(node.style[paddingSide]) > 0)
					return true;
			}

			return ed.queryCommandState('InsertUnorderedList') || ed.queryCommandState('InsertOrderedList') || (!settings.inline_styles && !!dom.getParent(selection.getNode(), 'BLOCKQUOTE'));
		}
	});
