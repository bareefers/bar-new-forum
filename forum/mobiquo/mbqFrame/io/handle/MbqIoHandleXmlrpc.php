...
Class MbqIoHandleXmlrpc {
    ...
    public static function alert($message, $result = false, $errorCode = NULL, $error_detail='') {
        header('Content-Type: text/xml');
        self::resetGlobals();
        $reason_data = $error_detail;
        if (is_array($reason_data) && isset($reason_data['reason'])) {
            if (isset($reason_data['error'])) {
                $error_detail = $reason_data['error'];
            }else{
                $error_detail = '';
            }
        }
        ...
        if (is_array($reason_data)) {
            if (isset($reason_data['result_url'])) {
                $response_array['result_url'] = new xmlrpcval($reason_data['result_url'], 'base64');
            }
            if (isset($reason_data['reason'])) {
                $response_array['result_reason'] = new xmlrpcval($reason_data['reason'], 'int');
            }
        }
        $response = new xmlrpcresp(new xmlrpcval($response_array, 'struct'));
        if (ob_get_length()) ob_end_clean();
        echo "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n".$response->serialize('UTF-8');
        exit;
    }
