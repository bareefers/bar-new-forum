...
class Dark_ParseHTML_ViewPublic_Helper_Message
{
	public static function getBbCodeWrapper(array &$message, XenForo_BbCode_Parser $parser, array $options = array())
	{
		...
		if(XenForo_Application::get('options')->currentVersionId < 1020031){
			...
			$wrapper = new Dark_ParseHTML_BbCode_TextWrapper($text, $parser, $options['states']);

		
		// >= v1.2	
		} else {
			
			$wrapper = new Dark_ParseHTML_BbCode_TextWrapper($text, $parser, $options['states'], array(
	...
	public static function bbCodeWrapMessages(array &$messages, XenForo_BbCode_Parser $parser, array $options = array())
	{
		...
		{
			$message['messageHtml'] = '';
			$message['messageHtml'] = Dark_ParseHTML_ViewPublic_Helper_Message::getBbCodeWrapper($message, $parser, $options);
		}
	}
}