...
class XenForo_ControllerPublic_Register extends XenForo_ControllerPublic_Abstract
{
	...
	public function actionRegister()
	{
		...
		if (!$errors && $ip && $options->get('registrationCheckDnsBl', 'check'))
		{
			$dnsBlCache = $this->getModelFromCache('XenForo_Model_DataRegistry')->get('dnsBlCache');
			if (!$dnsBlCache) {
				$dnsBlCache = array();
			}
			...
			{
				// seen before
				$block = $dnsBlCache[$ip]['block'];
				$log = false;
			}
			else if (XenForo_DnsBl::checkSpamhaus($ip) || XenForo_DnsBl::checkTornevall($ip))
			{
			...
			if ($block)
			{
				...
				{
					$errors[] = new XenForo_Phrase('ip_blacklisted_contact_to_register');
				}
				else
				{
			...
			if ($log)
			{
				...
				$this->getModelFromCache('XenForo_Model_DataRegistry')->set('dnsBlCache', $dnsBlCache);
			}
