...
class XenForo_Model_Template extends XenForo_Model
{
	...
	public function compileAndInsertParsedTemplate($templateMapId, $parsedTemplate, $title, $compileStyleId)
	{
		...
		$fileTemplates = XenForo_Application::get('options')->templateFiles;
		...
		if ($isCss)
		{
			...
			$db->query($insertStatement, array($compileStyleId, 0, $title, $compiledTemplate));
			if ($fileTemplates)
			{
				XenForo_Template_FileHandler::save($title, $compileStyleId, 0, $compiledTemplate);
			}
		}
		else
		{
			foreach ($languages AS $language)
			{
				...
				$db->query($insertStatement, array($compileStyleId, $language['language_id'], $title, $compiledTemplate));
				if ($fileTemplates)
				{
					XenForo_Template_FileHandler::save($title, $compileStyleId, $language['language_id'], $compiledTemplate);
				}
			}
		}
