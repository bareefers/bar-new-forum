...
class XenForo_Model_Conversation extends XenForo_Model
{
	...
	public function prepareConversationFetchOptions(array $fetchOptions)
	{
		...
		if (!empty($fetchOptions['join']))
		{
			...
			if ($fetchOptions['join'] & self::FETCH_RECEIVED_BY)
			{
				$joinTables .= '
			...
			//if ($fetchOptions['join'] & self::FETCH_REPLIED_BY)
			//{
			//	$joinTables .= '
			//		INNER JOIN xf_conversation_message AS check_message ON
			//			(check_message.conversation_id = conversation_master.conversation_id)';
			//}
		}
	...
	public function prepareConversationConditions(array $conditions, array &$fetchOptions)
	{
		...
		if (!empty($conditions['search_type']) && !empty($conditions['search_user_id']))
		{
			switch ($conditions['search_type'])
			{
				...
				case 'received_by':
				{
					$this->addFetchOptionJoin($fetchOptions, self::FETCH_RECEIVED_BY);
					$sqlConditions[] = 'check_recipient.user_id = ' . $db->quote($conditions['search_user_id']);
					break;
				}

				//case 'message_by':
				//{
				//	$this->addFetchOptionJoin($fetchOptions, self::);
				//	$sqlConditions[] = 'check_message.user_id = ' . $db->quote($conditions['search_user_id']);
				//}
			}
		}
	 ...
	 * @return array|false
	 */
	public function getAttachmentParams(array $conversation = array(), array $contentData = array(), array $viewingUser = null)
	{
